...
public class vPartition implements Serializable {
	...
	public vPartition(String uuid_string) throws IOException, ClassNotFoundException {
		...
		// Deserialize the File Allocation Table (FAT) and the root directory
	...
	public vPartition(char driveLabel, long partitionSize) throws Exception {
		...
		// Initialize File Allocation Table (FAT) and root directory
	 ...
	 *
	 * @return The deserialized vDirectory instance representing the root directory.
	 * @throws IOException If an I/O error occurs during the deserialization process.
	 * @throws ClassNotFoundException If the class of a serialized object cannot be found.
	 */
	public vDirectory deserializeRoot() throws IOException, ClassNotFoundException {
		// Create a byte array to store the serialized data of the root directory
		...
		// Deserialize the vDirectory instance from the byte array
		try (ByteArrayInputStream bais = new ByteArrayInputStream(obj);
		     ObjectInputStream ois = new ObjectInputStream(bais)) {
	...
	/**
	 * Creates a new vFile instance in the specified directory.
	 *
	 * @param directory The parent vDirectory instance.
	 * @param fileName The name of the new file.
	 * @param fileType The type (extension) of the new file.
	 * @return The newly created vFile instance, or null if the file already exists in the directory.
	 */
	 ...
	 *
	 * @param sourceDir The source vDirectory instance.
	 * @param sourceFile The file to be moved (vFile instance).
	 * @param destDir The destination vDirectory instance.
	...
	public void moveFile(vDirectory sourceDir, vFile sourceFile, vDirectory destDir, String destFile) {
		...
		// Delete the file entry from the source directory
		sourceDir.deleteEntry(sourceFile);

		// Create a new entry for the file in the destination directory
	 ...
	 *
	 * @param parent The parent vDirectory instance.
	 * @param folderName The name of the new folder.
	 */
	public void createFolder(vDirectory parent, String folderName) {
		// Create a new vDirectory object with the given name and parent
		vDirectory folder = new vDirectory(folderName, parent);

		// Check if the folder already exists in the parent directory
		if (parent.getFileStartBlock(folder) != null) {
			return; // Folder already exists, no need to create a new one
		}

		// Create a new entry for the folder in the parent directory
	...
	public void deleteFolder(vDirectory parent, String folderName) {
		...
		// Remove the folder entry from the parent directory
		parent.deleteEntry(folder);
	}

	/**
	 * Retrieves the content of a vFile from the specified vDirectory.
	 *
	...
	/**
	 * Saves data to a vFile instance within the specified vDirectory.
	 *
	 * @param directory The vDirectory instance containing the file.
	...
	public void saveFileData(vDirectory directory, vFile file, byte[] data) {
		...
		// Create an entry in the directory with the index of the first data block
		directory.createEntry(file, allocatedBlocks[0]);
	}

	/**
	 * Deletes the file data associated with a vFile in the specified directory.
	 *
	 * @param directory The vDirectory instance containing the file.
	 ...
	 * This method is typically used when deleting a directory, and it ensures that all
	 * resources within the directory are properly released.
	 *
	 * @param folder The vDirectory instance representing the directory to delete.
	 ...
	 *         Returns null if the path is invalid or the directory does not exist.
	 */
	...
	public String getPathString(vDirectory folder) {
		...
		// Iterate through parent directories and prepend each directory name
